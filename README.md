...
> One MCP endpoint. 673 tools. 204 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 204 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (673 tools, 204 providers)
...
| **Health & Nutrition** | 24 | USDA, OpenFDA, NIH, CDC, UK FSA, NIH Reporter, RxNorm, Europe PMC | Food data, drug safety, supplements, public health datasets, UK food hygiene ratings, NIH grant search, drug nomenclature (RxNorm) |
| **Chemistry & Biology** | 24 | PubChem, RCSB PDB, NCI CACTUS, Materials Project, MyGene, MyVariant, MyChem | 197M+ chemical compounds, 220K+ proteins, 150K+ materials, 450M+ annotated variants, gene annotation |
...
| **Education & Research** | 42 | OpenAlex, arXiv, PubMed, CrossRef, DBLP, Zenodo, DataCite | Papers, colleges, DOI lookup, CS bibliography, research datasets |
