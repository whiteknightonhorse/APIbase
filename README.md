...
> One MCP endpoint. 418 tools. 125 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Production MCP server that gives AI agents access to 418 real-world API tools through a single endpoint. Agents connect once to `https://apibase.pro/mcp` and can search flights, get stock quotes, translate text, check weather alerts, generate images, send emails, look up holidays, shorten URLs, detect fires by satellite, decode VINs, look up chemical compounds, find EV chargers, batch multiple calls, track usage analytics — and 250+ more tools across 30+ categories.
...
## Tool Categories (418 tools, 125 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 418 tools, JSON schemas)
...
**Progressive disclosure:** Instead of loading all 418 tool schemas into context, agents can call the `discover_tools` prompt to find relevant tools first:
