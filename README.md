...
> One MCP endpoint. 431 tools. 130 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
## Tool Categories (431 tools, 130 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 431 tools, JSON schemas)
...
**Progressive disclosure:** Instead of loading all 431 tool schemas into context, agents can call the `discover_tools` prompt to find relevant tools first:
