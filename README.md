...
> One MCP endpoint. 254 tools. 71 providers. Pay per call with x402 USDC on Base.
...
Production MCP server that gives AI agents access to 254 real-world API tools through a single endpoint. Agents connect once to `https://apibase.pro/mcp` and can search flights, get stock quotes, translate text, check weather alerts, generate images, send emails, look up holidays, shorten URLs, detect fires by satellite, decode VINs, look up food products — and 200+ more tools across 30+ categories.
...
## Tool Categories (254 tools, 71 providers)
...
| **Art & Culture** | 2 | Europeana | 50M+ EU cultural objects, 36 countries |
...
GET /api/v1/tools → Full tool catalog with schemas (all 254 in one response)
