...
> One MCP endpoint. 973 tools. 273 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 273 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (973 tools, 273 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 973 tools, JSON schemas)
