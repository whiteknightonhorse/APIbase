...
> One MCP endpoint. 741 tools. 221 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 221 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (741 tools, 221 providers)
...
| **Finance & Stocks** | 29 | Finnhub, CoinGecko, ECB, FRED, World Bank, WTO, Bank of Canada, MFAPI | Stock quotes, OHLCV, FX rates, economic data, global indicators, WTO trade/tariff statistics, CAD FX rates + policy rates + CPI, India mutual fund NAV data |
