...
> One MCP endpoint. 818 tools. 239 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 239 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (818 tools, 239 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 818 tools, JSON schemas)
