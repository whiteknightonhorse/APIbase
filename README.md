...
> One MCP endpoint. 729 tools. 218 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 218 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (729 tools, 218 providers)
...
| **Health & Nutrition** | 28 | USDA, OpenFDA, NIH, CDC, UK FSA, NIH Reporter, RxNorm, Europe PMC, PharmGKB | Food data, drug safety, supplements, public health datasets, UK food hygiene ratings, NIH grant search, drug nomenclature (RxNorm), pharmacogenomics (gene/drug/variant/drug labels) |
