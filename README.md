...
> One MCP endpoint. 1116 tools. 309 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 309 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (1116 tools, 309 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 1116 tools, JSON schemas)
