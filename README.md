...
> One MCP endpoint. 733 tools. 219 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 219 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (733 tools, 219 providers)
...
| **Finance & Stocks** | 25 | Finnhub, CoinGecko, ECB, FRED, World Bank, WTO, Bank of Canada | Stock quotes, OHLCV, FX rates, economic data, global indicators, WTO trade/tariff statistics, CAD FX rates + policy rates + CPI |
...
| **Health & Nutrition** | 32 | USDA, OpenFDA, NIH, CDC, UK FSA, NIH Reporter, RxNorm, Europe PMC, PharmGKB, OpenFDA Devices | Food data, drug safety, supplements, public health datasets, UK food hygiene ratings, NIH grant search, drug nomenclature (RxNorm), pharmacogenomics, FDA device recalls/510k/MAUDE |
