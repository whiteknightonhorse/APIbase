...
> One MCP endpoint. 925 tools. 262 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 262 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (925 tools, 262 providers)
...
GET  /api/v1/tools                      → Live tool catalog (all 925 tools, JSON schemas)
