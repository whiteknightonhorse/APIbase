...
> One MCP endpoint. 393 tools. 114 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
## Tool Categories (393 tools, 114 providers)
...
GET  /api/v1/tools             → Full tool catalog (all 393 tools in one response)
GET  /health/ready             → System health check
POST /mcp  prompts/get discover_tools  → Browse 393 tools by category or task (progressive disclosure)
```

**Progressive disclosure:** Instead of loading all 393 tool schemas into context, agents can call the `discover_tools` prompt to find relevant tools first:
