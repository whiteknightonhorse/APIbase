...
> One MCP endpoint. 637 tools. 195 providers. Pay per call with x402 (USDC on Base) or MPP (USDC on Tempo).
...
Zero API-key management (Base Account handles auth + signing), zero per-provider signup (APIbase aggregates 195 providers). Full guide: [apibase.pro/frameworks#base-mcp](https://apibase.pro/frameworks#base-mcp).
...
## Tool Categories (637 tools, 195 providers)
...
| **Health & Nutrition** | 16 | USDA, OpenFDA, NIH, CDC, UK FSA, NIH Reporter | Food data, drug safety, supplements, public health datasets, UK food hygiene ratings, NIH grant search |
...
| **Space & Astronomy** | 17 | NASA, JPL, NOAA SWPC, NTRS | APOD, asteroids, fireballs, solar flares, technical reports |
