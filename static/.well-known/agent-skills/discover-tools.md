---
name: discover-tools
description: Progressive-disclosure workflow for finding relevant tools in APIbase's 1377-tool catalog without loading every schema into the agent context.
---

# Skill: Discover tools on APIbase

APIbase exposes 1377 tools across 25 categories. Loading all schemas into context
...
- The MCP server has more than ~50 tools (APIbase has 1377).
...
4. **Call the chosen tool via `tools/call`.** All 1377 tools are always callable —
...
Returns all 1377 tools with full JSON Schemas. Use sparingly — the response is
large.
