---
name: discover-tools
description: Progressive-disclosure workflow for finding relevant tools in APIbase's 1388-tool catalog without loading every schema into the agent context.
---

# Skill: Discover tools on APIbase

APIbase exposes 1388 tools across 25 categories. Loading all schemas into context
...
- The MCP server has more than ~50 tools (APIbase has 1388).
...
4. **Call the chosen tool via `tools/call`.** All 1388 tools are always callable —
...
Returns all 1388 tools with full JSON Schemas. Use sparingly — the response is
large.
